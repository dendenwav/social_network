...
const getUsers = async (req, res) => {
    try {
        const users = await UserModel_1.default.find({}, { email: 0, password: 0, updatedAt: 0 });
...
const getUser = async (req, res) => {
    ...
    try {
        const user = await UserModel_1.default.findOne({ pseudo });
        if (!user)
...
const updateUser = async (req, res) => {
    try {
        ...
        const userToUpdate = await UserModel_1.default.findOne({ pseudo: req.body.userId });
        if (!userToUpdate)
...
const deleteUser = async (req, res) => {
    try {
        ...
        if (!currentUser)
            return res.status(404).json({ message: "currentUser not found." });
        const userToDelete = await UserModel_1.default.findOne({ pseudo: req.params.id });
        if (!userToDelete)
            return res.status(404).json({ message: "User not found." });
        if (currentUser.pseudo === userToDelete.pseudo || currentUser.isAdmin) {
            await UserModel_1.default.findOneAndDelete({ pseudo: userToDelete.pseudo });
...
const followUser = async (req, res) => {
    try {
        ...
        if (!currentUser)
            return res.status(404).json({ message: "currentUser not found." });
        const userToFollow = await UserModel_1.default.findOne({ pseudo: req.params.id });
        if (!userToFollow)
            return res.status(404).json({ message: "User not found." });
...
const unfollowUser = async (req, res) => {
    try {
        ...
        if (!currentUser)
            return res.status(404).json({ message: "currentUser not found." });
        const userToUnfollow = await UserModel_1.default.findOne({ pseudo: req.params.id });
        if (!userToUnfollow)
            return res.status(404).json({ message: "User not found." });
